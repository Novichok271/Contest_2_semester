    ...
    if (fscanf(in, "%d %d\n", n, monomsCount) == 2 && (*n > 0 && *monomsCount > 0))
    {
        ...
        for (int i = 0; i != _monomsCount; ++i)
        {
            for (int j = 0; j != _n + 2; ++j)
            {
                ...
                //Ключевой момент в контесте после m и n стоит ещё символ '\r'
                //Поэтому я _n + 2 раза бегаю по циклу
                // Для наглядности 00**'\r' '\n'
                //Именно так и всё и устроено в контесте
                //Если вы проверяете чисто для себя на vs code или vs, то просто 
                //сделайте количество итераций _n + 1
                {
                    if (sym == '*')
                    {
                        arrayOfMonoms[i][j] = '2';
